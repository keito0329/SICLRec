...
class Trainer:
    def __init__(self, model, train_dataloader, eval_dataloader, test_dataloader, args, logger):
        ...
        # # == 追加: FrequencyLayer 参照をキャッシュ ==
        # # BSARecEncoder は blocks → 各 block は BSARecLayer → filter_layer が FrequencyLayer
        # self.freq_layer = None
        # for block in self.model.item_encoder.blocks:
        #     if hasattr(block.layer, 'filter_layer'):
        #         self.freq_layer = block.layer.filter_layer
        #         break
        # if self.freq_layer is None:
        #     raise ValueError("FrequencyLayer が見つかりませんでした。")
    ...
    def train(self, epoch):
        ...
        # == 追加: エポック開始前にリセット ==
        # self.freq_layer.reset_spectrum_stats()

        self.iteration(epoch, self.train_dataloader, train=True)

        # == 追加: エポック終了後に平均スペクトルをプロット ==
        # self.freq_layer.plot_avg_spectrum(epoch)
    ...
    def iteration(self, epoch, dataloader, train=True):
        ...
        if train:
            ...
            for i, batch in rec_data_iter:
                ...
                # リハーサルバッファが有効な場合、毎バッチ更新を試みる
                # if self.args.use_rehearsal_buffer:
                #     batch_data = (user_ids, input_ids, answers, neg_answer, same_target)
                #     self.update_rehearsal_buffer(batch_data, loss.item())
            ...
            # エポック終了後、リハーサルバッファ内のデータの10%（最低1サンプル）をランダムサンプリングして再学習
            # if self.args.use_rehearsal_buffer and len(self.rehearsal_buffer) > 0:
            #     sample_size = max(1, int(len(self.rehearsal_buffer) * 0.1))
            #     rehearsal_samples = random.sample(self.rehearsal_buffer, sample_size)
            #     for rehearsal_sample in rehearsal_samples:
            #         user_ids_r, input_ids_r, answers_r, neg_answer_r, same_target_r, _ = rehearsal_sample
            #         rehearsal_loss = self.model.calculate_loss(input_ids_r, answers_r, neg_answer_r, same_target_r, user_ids_r)
            #         self.optim.zero_grad()
            #         rehearsal_loss.backward()
            #         self.optim.step()
                    # self.logger.info(f"Rehearsal update: loss = {rehearsal_loss.item():.4f}")
